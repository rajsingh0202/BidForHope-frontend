...
      // Wallet amount: only 'debited' credits minus 'debited' debits
      const creditedBalance = txs
        .filter(tx => tx.type === 'credit' && tx.status === 'debited')
        .reduce((acc, tx) => acc + tx.amount, 0);
      const withdrawalTotal = txs
        .filter(tx => tx.type === 'debit' && tx.status === 'debited')
        .reduce((acc, tx) => acc + tx.amount, 0);
...
    if (!ngoEmail) { setBankDetails(null); return; }
    try {
      const res = await axios.get(`${API_BASE_URL}/api/ngos/bank-details`, { params: { email: ngoEmail } });
...
      });
    } catch (error) { setBankDetails(null); }
  };

  // Fetch Withdrawal Requests for this NGO
  const fetchWithdrawalRequests = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/withdrawals/my-requests`, { params: { ngoEmail } });
      setWithdrawalRequests(Array.isArray(res.data.data) ? res.data.data : []);
    } catch (error) { setWithdrawalRequests([]); }
  };
...
        ngoEmail, amount: Number(amount), domain, description
      });
      setAmount(''); setDomain(''); setDescription('');
...
    try {
      await axios.put(`${API_BASE_URL}/api/ngos/bank-details`, { email: ngoEmail, ...bankForm });
...
  useEffect(() => {
    fetchTransactions();
    if (ngoEmail) { fetchBankDetails(); fetchWithdrawalRequests(); }
...
        fetchWithdrawalRequests(); fetchTransactions();
      }
    });
...
          Pending credits (not available to withdraw): ₹{pendingAmount.toLocaleString()}
        </div>
      )}
...
                  <td className="px-2 py-2 font-bold">
                    {tx.status === 'pending' && (
                      <span className="bg-yellow-700 text-yellow-200 px-2 py-1 rounded-full">Pending</span>
                    )}
                    {tx.status === 'debited' && (
                      <span className="bg-green-700 text-green-200 px-2 py-1 rounded-full">Credited</span>
                    )}
                    {!tx.status && (
...
                <tr>
                  <td colSpan={7} className="py-7 text-center text-gray-300 text-sm">
